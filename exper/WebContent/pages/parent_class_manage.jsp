    ...
    <script type="text/javascript">
		...
		function class_add(){
			...
			$("#class_save").unbind('click').click(function(){
  				$.ajax({
  					type: 'post',
					url : '${pageContext.request.contextPath}/pclass/class_add',
		...
		function class_edit(){
    		...
    		if(row){
				...
				$("#class_save").unbind('click').click(function(){
  					$.ajax({
  						type:'post',
						url : '${pageContext.request.contextPath}/pclass/class_update',
						data : $('#class_form').serialize(),
						dataType : 'json',
						success : function(obj) {
							if (obj.success) {
    			...
    			$.messager.confirm(
    				...
    				function(r) {
    					if (r) {
    						$.ajax({ 
    							type:'post',
    			    			url: '${pageContext.request.contextPath}/pclass/class_delete',
    			    			data : {"id":row.id},
    			    			dataType : 'json',
    			    			success : function(obj){
    			    				if(obj.success){
    		...
    		<div class="form-group col-md-12">
             ...
             <div class="form-group col-md-12" >
                     <label  class="col-md-4" style="display: inline-block;height: 34px;line-height: 34px;text-align: left;width: 30%">开始时间:</label>
                		<input class="form-control easyui-datetimebox"  name="startTime" style="display: inline-block;width: 70%" />
                  </div>
            <div class="form-group col-md-12">
            	<label class="col-md-4" style="display: inline-block;height: 34px;line-height: 34px;text-align: left;width: 30%">结束时间：</label>
                <input name="endTime" class="form-control easyui-datetimebox"  style="display: inline-block;width: 70%"/>
            </div>
            <div class="form-group col-md-12">
